...
class GameScreen_quanxian(GameScreen_init, QMainWindow):
    def __init__(self):
        ...
        self.comboBox_list_quanxian = \
            ["普通用户", "音乐管理员", "游戏管理员", "用户管理员"]  # 设置权限选择框中内容
    ...
    def data_init_quanxian(self):
        ...
        data = self.database_quanxian.select("administrator_table") + \
               self.database_quanxian.select("user_table")
        # 遍历data列表
        # enumerate()函数用于将一个可遍历的数据对象(如列表、元组或字符串)组合为一个索引序列
        for index, item in enumerate(data):
            # 设置普通用户账户类型
            if item[2] in self.comboBox_list_quanxian or item[2] == "系统管理员":
                item_quanxian = item
                self.saveList_quanxian_admin.append(item_quanxian)   # 管理员存储列表添加数据元组
            else:
                item_quanxian = (item[0], item[1], "普通用户")
                self.saveList_quanxian_user.append(item_quanxian)    # 用户存储列表添加数据元组
            self.newLine_quanxian(index + 1, item=item_quanxian)  # 表格新增1行
    ...
    def newLine_quanxian(self, num, item=None):
        ...
        if num == 1:
            ...
            # 设置选择框普通用户/音乐管理员/游戏管理员/用户管理员,批量化生产子控件
        ...
        for item in self.displayList_quanxian:
            if item not in self.saveList_quanxian_admin + self.saveList_quanxian_user:
                ...
                if item[0] in admin_nameList:
                    ...
                    if item[2] == "普通用户":
                        self.database_quanxian.delete("administrator_table", [item[0]])
                        self.database_quanxian.insert("user_table", [item[0], item[1], False])
                        print('管理员' + item[0] + '变更为普通用户')
                    # 若未被变更为普通用户,则在管理员表中更新其内容
                    else:
                        self.database_quanxian.update("administrator_table", [item[0], item[1], item[2]])
                        print('管理员' + item[0] + '被更新')
                # 若修改的账户原来是普通用户
                elif item[0] in user_nameList:
                    # 若该用户未被设置成管理员,则在用户表中更新其内容
                    if item[2] == "普通用户":
                        self.database_quanxian.update("user_table", [item[0], "all", item[1], False])
                        print('用户' + item[0] + '被更新')
                    # 若该用户被设置成管理员,则在用户表中剔除,在管理员表中新增
                    else:
                        self.database_quanxian.delete("user_table", [item[0]])
                        self.database_quanxian.insert("administrator_table", [item[0], item[1], item[2]])
