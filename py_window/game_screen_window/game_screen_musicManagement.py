...
from PyQt5.QtWidgets import QTableWidgetItem, QMainWindow, QHeaderView, QFileDialog
...
class GameScreen_musicManagement(GameScreen_init, QMainWindow):
    def __init__(self):
        ...
        # 数据库的设置
        self.database_musicManagement = Database_root()
        # 音乐表的设置
    ...
    def data_init_music(self):
        # 获取sel语句的数据列表
        data = self.database_musicManagement.select("music_table")
        # 遍历data列表
        ...
        for item in self.displayList_music:
            if item not in self.saveList_music:
                ...
                if item[0] not in idList:
                    ...
                    self.database_musicManagement.update("music_table", [item[0], item[1], item[2]])
                    print("update music")
        for item in self.saveList_music:
            if item[0] not in _idList:
                self.database_musicManagement.delete("music_table", [item[0]])
